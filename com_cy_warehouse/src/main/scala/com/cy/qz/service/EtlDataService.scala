...
object EtlDataService {
  /**
    ...
    * @param sparkSession
    */
  def etl1QzWebsite(ssc:SparkContext,sparkSession: SparkSession): Unit ={
    ...
    ssc.textFile("/user/atguigu/ods/QzWebsite.log").filter(item=>{
      ...
      partition.map(item=>{
        ...
        (siteid,sitename,domain,sequence,multicastserver,templateserver,status,creator,createtime,multicastgateway,multicastport,dt,dn)
      })
  ...
  def etlQzPoint(ssc:SparkContext,sparkSession: SparkSession): Unit ={
    ...
    ssc.textFile("/user/atguigu/ods/QzPoint.log").filter(item => {
      ...
      partition.map(item=>{
        ...
        //jsonObject.getBigDecimal("score").divide()
